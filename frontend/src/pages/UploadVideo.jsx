...
const UploadVideo = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    const handleChange = (e) => {
        ...
        else if (name === "videoFile") { // ✅ match input name
            const file = files[0];
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.tag) uploadData.append("tags", formData.tag);

        if (formData.videoFile) uploadData.append("videoFile", formData.videoFile);  // ✅
        if (formData.thumbnail) uploadData.append("thumbnail", formData.thumbnail); // ✅

        try {
            const res = await axios.post("/videos/", uploadData, {
            ...
            <div className={`min-h-screen w-full bg-base-200 p-5 md:p-10 text-base-content pr-0 md:pr-3 pt-2 pb-2 gap-3 duration-500 flex flex-col ${sidebarOpen ? 'pl-0 md:pl-77' : 'pl-0 md:pl-25'}`}>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-10 w-full mx-auto">
                    ...
                    <div className="flex flex-col gap-6 w-full">
                        ...
                        <button type="submit" className="btn btn-primary mt-35 w-1/3 mx-auto text-lg" disabled={loading}>

                            {loading ? (
                                // Simple spinner SVG or any loader you like
